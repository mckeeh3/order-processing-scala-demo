...
class StockSkuItemToOrderSkuItemAction(creationContext: ActionCreationContext) extends AbstractStockSkuItemToOrderSkuItemAction {

  override def onStockRequestedJoinToOrder(stockRequestedJoinToOrder: StockRequestedJoinToOrder): Action.Effect[Empty] = {
    throw new RuntimeException("The command handler for `OnStockRequestedJoinToOrder` is not implemented, yet")
  }
